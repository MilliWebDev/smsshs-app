                    ...
                    <form class="mt-5" wire:submit="createSubject">
                        ...
                        <div class="mt-4">
                            <label for="coefficient"
                                class="block text-lg font-bold text-gray-700 capitalize dark:text-gray-200">{{ __('Coefficient de la matière') }}</label>
                            <input placeholder="1" type="number" wire:model="subject_coefficient"
                                class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40">
                            @error('coefficient')
                                <p class="mt-1 text-sm text-red-600">{{ __('Veillez entrer un coefficient') }}</p>
                            @enderror
                        </div>
                ...
                @else
                    @foreach ($class_subject_teachers->groupBy('subject_id') as $subject_id => $items)
                        <tr class="border" wire:key="{{ $subject_id }}">
                            ...
                            <td class="px-4 py-2 text-lg font-bold">
                                {{ $items->first()->subject->coefficient }}
                            </td>
                            <td class="px-4 py-2 ">
                                <ol>
                                                ...
                                                <form class="mt-5" wire:submit.prevent="update">
                                                    ...
                                                    <div class="mt-4">
                                                        ...
                                                        <input placeholder="1" type="number"
                                                            wire:model="subject_coefficient"
                                                            class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40">
                                                        @error('coefficient')
                                                            <p class="mt-1 text-sm text-red-600">
                                                                {{ __('Veillez entrer un coefficient') }}</p>
                                                        @enderror
                                                    </div>
